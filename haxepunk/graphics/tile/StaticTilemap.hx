 ...
 */
class StaticTilemap extends Graphic
{
	/**
    ...
    _texture = Texture.create(width, height, true);
    _region = Atlas.loadImageAsRegion(_texture);
    ...
    if(index == -1) {
      _texture.clearRect(new flash.geom.Rectangle(column * tileWidth, row * tileHeight, tileWidth, tileHeight));
      return;
    }
    var region = _atlas.getRegion(index);
    var rect = new flash.geom.Rectangle(region.x, region.y, region.width, region.height);
    _texture.drawTexture(column * tileWidth, row * tileHeight, @:privateAccess _atlas._data.texture, rect);
	}
